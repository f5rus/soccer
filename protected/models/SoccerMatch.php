...
class SoccerMatch extends CActiveRecord
{
    ...
    public static function getResults($matchday=0){
        $date=date("Y-m-d 00:00:00",strtotime("now"));
        if ($matchday==0) {
            $matches=SoccerMatch::model()->FindAll('status="FT"');
        } else {
            $matches=SoccerMatch::model()->FindAll('status="FT" AND matchday=:md',array('md'=>$matchday));
        }
        $games=array(); $k=0;
    ...
    public static function getFixtures($matchday=0){
        $date=date("Y-m-d 00:00:00",strtotime("now"));
        if ($matchday==0) {
         $matches=SoccerMatch::model()->FindAll('date>:date',array('date'=>$date));
        } else {
            $matches=SoccerMatch::model()->FindAll('date>:date AND matchday=:matchday',array('date'=>$date,'matchday'=>$matchday));
        }
        $games=array(); $k=0;
        for ($i=0;$i<count($matches);$i++) {
            $games[$k++]=$matches[$i];
