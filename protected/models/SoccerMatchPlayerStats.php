...
class SoccerMatchPlayerStats extends CActiveRecord
{
	...
	public function relations()
	{
		...
		return array(
            'player' => array(self::BELONGS_TO, 'SoccerPlayer', 'player_id'),
    ...
    public static function getPlayerStats($player_id,$count=false) {
        //$date=date("Y-m-d 00:00:00",strtotime("now"));
        $matches=SoccerMatchPlayerStats::model()->FindAll('player_id=:pid',array('pid'=>$player_id));
        ...
        $stats['games']=count($matches);

        for($i=0;$i<count($matches);$i++) {
            if (isset($matches[$i]->goals)) $stats['goals']+=$matches[$i]->goals;
            if (isset($matches[$i]->assists)) $stats['assists']+=$matches[$i]->assists;
            if (isset($matches[$i]->yellowcards)) $stats['yellowcards']+=$matches[$i]->yellowcards;
            if (isset($matches[$i]->redcards)) $stats['redcards']+=$matches[$i]->redcards;
            if (isset($matches[$i]->pen_score)) $stats['pen_score']+=$matches[$i]->pen_score;
            //if (isset($matches[$i]->goals)) $stats['games']++;
        }
