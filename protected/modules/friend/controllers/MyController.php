...
class MyController extends Controller
{
    ...
    public function actionFriends() {
        if (isset($_GET['action'])){
            $id=$_GET['action'];
            //$users= User::model();
            $listing = User::model()->with(array('friend' => array('alias' => 'pu')))->FindAll(
                array('condition'=>"pu.user1=:id AND pu.relation=2",'params'=>array(":id"=>$id))
            );
            //print_r($listing[0]); exit();
            //
            //, 'params'=>array(":id"=>$id),'limit'=>40


            if (count($listing)==0) {
                echo "У пользователя пока нет друзей";
            }
            else {
                echo "<ul class=\"userlist_mini\">";
                foreach($listing as $user){
                    //echo $user->profile->firstname."**<br>";
                    $this->renderPartial("application.modules.user.views.user._viewmini",array('data'=>$user));
                }
                echo "</ul>";
            }
            Yii::app()->end();
