...
class Forecast extends CActiveRecord
{
	...
	public function search()
	{
        ...
        //статистические данные
         public static function current_match(){
            $current_match = Yii::app()->db->createCommand('SELECT id FROM tbl_soccer_match WHERE date<= NOW() ORDER BY date DESC LIMIT 1')->queryScalar(); 
        ...
        public static function count_all(){
            $sqlcommand=  Yii::app()->db->createCommand('SELECT COUNT(*) FROM tbl_soccer_match');                 
            $count=  $sqlcommand->queryScalar();    
        ...
        {
            $match = self::model()->current_match();
            $model = self::model()->findAll('match_id<=:match_id AND scores IS NULL', array(':match_id'=>$match));
            foreach($model as $row){
                if (is_null($row->match->status)){
                    continue;
                }
                ...
                $homegoals_ = $row->homegoals;
                $awaygoals_ = $row->awaygoals;


                if(($homegoals == $homegoals_) && ($awaygoals==$awaygoals_))
                {
                    $scores = 5;
                }
                elseif(($homegoals-$awaygoals)==($homegoals_-$awaygoals_)) 
                {
                    $scores = 3;
                }
                elseif((($homegoals > $homegoals_) && ($awaygoals>$awaygoals_)) ||
                      (($homegoals < $homegoals_) && ($awaygoals<$awaygoals_)) ||
                      (($homegoals==$homegoals_) && ($awaygoals==$awaygoals_)))
                {
                    $scores = 2;
                }
